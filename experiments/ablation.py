...
def make_cpm(dim: int, variant: str = "3-tier", consolidate: bool = True):
    ...
    if not consolidate:
        cfg.consolidate_hit_count = 10**9
...
def exp_capacity(ax):
    ...
    for dim in dims:
        nmax = int(2.5 * dim)
        checks = list(range(dim // 4, nmax + 1, max(1, dim // 4)))
...
def exp_recall_mode(ax):
    ...
    for i in range(nmax):
        ...
        if (i + 1) in checks:
            xs.append(i + 1)
            assoc.append(eval_acc(cpm, protos, i + 1, mode="assoc"))
            proto.append(eval_acc(cpm, protos, i + 1, mode="proto"))
            hybrid.append(eval_acc(cpm, protos, i + 1, mode="hybrid"))
